...
begin
  ...
  begin
    //ShowMessage('ERROR: The parameter file does not exist! Full Path: ' + ExpandFileName(paramname));
    Halt(1)
  end;

   //ShowMessage('DEBUG: Checking file at ' + ExpandFileName(paramname));
   Assign(filename, paramname);
   reset(filename);
   //ShowMessage('DEBUG: File opened successfully: ' + paramname);
   ...
   //ShowMessage('DEBUG: Mapname read from parameters: '+ mapname);
...
begin
  ...
  begin
    ...
    WriteLn(csvFile,
            Famtree[i, 0]:0:0, ',',   // UniqueID data in ind i for the first column 0
            IC:0:3, ',',          // IC (Coefficient of Inbreeding), written with  2 decimals
